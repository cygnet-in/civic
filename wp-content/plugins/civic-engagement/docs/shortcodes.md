...
| `[civic_search_form]` | Search | Public search form. |
| `[civic_search_results]` | Search | Grouped public search results. |
...
| Search | `[civic_search_form]` and `[civic_search_results]` |
...
[civic_search_results limit="10" include_pages="0"]
```

The search form submits the public query string parameter `civic_search`. By default it submits to the configured Search Results Page from Civic Manager > System > Security. That page should contain `[civic_search_results]`. If no Search Results Page is configured, the form falls back to submitting to the current page.

Results are grouped by Consultations, Events, Schedules, and WordPress Posts/Pages. Civic module results use canonical public URLs.
