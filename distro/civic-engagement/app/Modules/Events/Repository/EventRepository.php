...
class EventRepository extends BaseRepository
{
    ...
    /**
     * Search public events for the shared Civic search experience.
     *
     * This intentionally searches only public-facing fields and includes
     * published and closed public events.
     *
     * @param string $keyword Search keyword.
     * @param array<string, mixed> $args Search arguments.
     * @return array<string, mixed> Paginated result set and metadata.
     */
    public function searchPublic(string $keyword, array $args = []): array
    {
        $keyword = trim($keyword);

        if ('' === $keyword) {
            return array_merge(['items' => []], $this->buildPaginationMeta(0, $this->parsePaginationArgs($args)));
        }
        ...
        $pagination = $this->parsePaginationArgs($args);
        $where = $this->buildEventFilters($args);
        $where['sql'][] = 'status IN (%s, %s)';
        $where['values'][] = 'published';
        $where['values'][] = 'closed';
        $search = $this->buildSearchClause($keyword, ['title', 'slug', 'summary', 'description', 'location']);
    ...
    /**
     * Get all events matching export filters without pagination.
     *
