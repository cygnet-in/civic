...
The Search Results Page setting selects the WordPress page that contains `[civic_search_results]`. Public `[civic_search_form]` output submits to that page while preserving the `civic_search` query parameter. If no page is configured, search forms submit to the current page.
