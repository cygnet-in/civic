...
Public search is provided by `SearchService` and the `[civic_search_form]` / `[civic_search_results]` shortcodes. The service coordinates consultations, events, schedules, and WordPress posts/pages. Module repository methods own the public search SQL and the service preserves canonical Civic URLs for module results.
