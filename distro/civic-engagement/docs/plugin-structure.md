...
- `CivicSettingsService`
  - stores shared Civic settings in WordPress options
  - currently manages CAPTCHA enabled state, Cloudflare Turnstile keys, and the public Search Results Page setting
- `CaptchaService`
...
- `SearchService`
  - coordinates public search across consultations, events, schedules, and WordPress posts/pages
  - delegates Civic module search SQL to module repositories
  - normalizes grouped result data for frontend shortcodes
  - preserves canonical Civic URLs for module results
...
- Shared public search across Civic content and WordPress content.
...
- uses the `civic_search` query parameter
- submits search forms to the configured Search Results Page when set
- groups results by consultations, events, schedules, and WordPress posts/pages
- calls module repository `searchPublic()` methods for Civic records
- uses canonical module URLs for Civic results
