...
| `[civic_search_form]` | Search module | Public search form. |
| `[civic_search_results]` | Search module | Grouped public search results. |
...
Search shortcodes use the `civic_search` query parameter. The search form submits to the configured Search Results Page when one is selected in Civic Manager settings; otherwise it submits to the current page. `SearchService` groups public results by consultations, events, schedules, and WordPress posts/pages. Civic result URLs use the canonical module routes.
...
- `civic-search` form and grouped result markup.
