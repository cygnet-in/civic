...
/**
 * Repository for configurable thread response fields.
 *
 * Handles civic_thread_fields database operations only. Rendering and workflow
 * decisions belong in controllers/services/templates.
 */
class ThreadFieldRepository extends BaseRepository
{
    /**
     * Supported thread field types.
     *
    ...
    {
        parent::__construct($wpdb, 'civic_thread_fields');
    }

    /**
     * Create a dynamic thread field.
     *
     * @param array<string, mixed> $data Field data keyed by civic_thread_fields columns.
    ...
    /**
     * Get fields for a thread ordered by sort order.
     *
    ...
    public function findByThreadId(int $threadId): array
    {
        ...
        $items = $this->wpdb->get_results(
            $this->prepare(
                "SELECT * FROM {$this->table} WHERE thread_id = %d ORDER BY sort_order ASC, id ASC",
                [$threadId]
            ),
            ARRAY_A
        );
